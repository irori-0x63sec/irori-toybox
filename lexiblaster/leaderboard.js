...
(function(){
  ...
  // 明示(window)指定
  const explicitFromWindow =
    typeof window !== 'undefined'
      ? (window.LEXI_LEADERBOARD_BASE || window.LEXI_LEADERBOARD_ENDPOINT || window.LEADERBOARD_API_BASE)
      : null;

  // 候補の優先順位を見直し：
  // 1) <script data-api-base> 系（最優先）
  // 2) window の明示値
  // 3) 既定: Cloudflare Workers の公開ドメイン
  // 4) /api/leaderboard は window 明示があるときだけ（ローカル開発用）
  const baseCandidates = [
    ...
    explicitFromWindow ? (typeof location !== 'undefined' ? `${location.origin}/api/leaderboard` : null) : null,
  ].filter(Boolean);
  ...
  try { console.info('[Leaderboard] API_BASE =', API_BASE); } catch {}
  ...
  async function handleSubmit(){
    ...
    try {
      // ゲーム情報（例）: mode/level/weak を detail.meta から拾う
      const mode  = String(state.meta?.mode  || '');
      const level = String(state.meta?.level || '');
      const weak  = !!state.meta?.weak;
      const game  = 'lexi-blaster';

      await submitScore({ name, score, mode, level, weak, game });
      saveStoredName(name);
  ...
  async function refreshLeaderboard(options = {}){
    ...
    try {
      // 同一モード/レベルで絞る（UI 側の meta を利用）
      const mode  = String(state.meta?.mode  || '');
      const level = String(state.meta?.level || '');
      const game  = 'lexi-blaster';
      const entries = await fetchLeaderboard({ limit, game, mode, level });
  ...
  async function fetchLeaderboard({ limit = 10, game = 'lexi-blaster', mode = '', level = '' } = {}){
    ...
    if (Number.isFinite(limit) && limit > 0) u.searchParams.set('limit', Math.floor(limit));
    if (game)  u.searchParams.set('game',  String(game));
    if (mode)  u.searchParams.set('mode',  String(mode));
    if (level) u.searchParams.set('level', String(level));
    ...
    const list = Array.isArray(data) ? data : (data?.items || data?.results || data?.top || data?.data || []);
  ...
  function normalizeEntry(item){
    ...
    const timestamp = Number(item?.timestamp ?? item?.ts);
  ...
  if (typeof window !== 'undefined') {
    window.lexiLeaderboard = {
      submitScore: (payload) => submitScore(payload),
      fetchLeaderboard: (params) => fetchLeaderboard(params),
      sanitizeName,
      clampScore,
